...
- Add explicit schemas and data-quality checks for identifiers, dates, prices, quantities, and totals.
- Parameterize catalog, schema, volume, input, and output paths.
- Add product-level metrics and year-aware monthly trends.
- Persist curated analytical tables as Delta rather than Parquet when using Databricks.
- Add automated PySpark tests for transformations and aggregations.
- Separate reusable transformations from notebook presentation code.
clear
